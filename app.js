...
scene('main', (levelId) => {
  ...
  addLevel(levels[levelId], {
    ...
    any(ch) {
      ...
      if (character) {
        return [sprite(character.sprite), area(), solid(), 'character', { msg: character.msg }]
      }
    },
  })
  ...
  function addDialog() {
    ...
    const txt = add([text('', { width: width() }), pos(0 + pad, height() - h + pad), z(100)])
  ...
  for (const dir in dirs) {
    ...
    keyDown(dir, () => {
      player.move(dirs[dir].scale(MOVE_SPEED))
    })
  }
})
